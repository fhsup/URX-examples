...
class RobotiqScript(URScript):
    ...
    def _rq_get_var(self, var_name):
        self._socket_get_var("{}".format(var_name), self.socket_name)

    def _get_gripper_fault(self):
        self._rq_get_var(FLT)


    def _get_gripper_object(self):
        self._rq_get_var(OBJ)

    def _get_gripper_status(self):
        self._rq_get_var(STA)

    def _get_gripper_pos(self):
        self._rq_get_var(POS)
...
class Robotiq_Two_Finger_Gripper(object):
    ...
    def send_opening(self, host, port):
        '''
        :param host: IP of the remote host (this computer)
        :param port: port used for tcp/ip connection
        :return: length between finger's gripper
        '''
        urscript = self._get_new_urscript()
        urscript._send_gripper_opening(host, port)
        time.sleep(0.1)
